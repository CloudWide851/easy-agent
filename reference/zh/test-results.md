...
- 最新一轮未发布验证发生在 2026 年 4 月 30 日：保留 4 月 14 日的 benchmark 与 public-eval headline 分数快照，同时刷新 Python verification 与 workflow/browser/dashboard/failure-to-fix CLI 检查，并覆盖新的 `runs inspect`、`workflow doctor/plan`、browser SEO/a11y/link plans 与 Python facade workflow/browser/bundle helpers；live provider compatibility matrix 仍保留 4 月 20 日验证快照。
...
- 静态检查：`ruff` 与 `mypy`
- 定向回归：setup preflight、wizard、config explanation、config doctor、可搜索 HTML trace export、本地 trace 打开、latest-report 汇总和 HTML export、静态 dashboard export、suggested dashboard next steps、可复制 dashboard inspect/bundle 命令、dashboard workflow/template recommendations、report trend、experimental OTel trace export、connector diagnostics、workflow packs、workflow YAML init/doctor/plan/run、run triage/inspect/bundle、Playwright MCP browser readiness、browser doctor/artifact/smoke/snapshot/audit/seo/a11y/links/report helpers、browser task packs、本地 skill catalog、plugin doctor、Python facade workflow/browser/bundle embedding、mock provider、onboarding CLI、scenario creation、新业务模板、run explanation/fix packages、provider compatibility、config validation、guardrails、BFCL evaluation、official-source search 与 simple-evals profile support；最新 focused set 为 `32 passed`，保留兼容性集合为 `55 passed`、`23 passed`、`36 passed`、`89 passed` 加 `4 passed`
- 全量 unit tests：`232 passed`
...
- 新增 focused regressions 覆盖 setup preflight、wizard scenario creation、模板 `workflow.yml` 生成、browser template Playwright MCP config、browser doctor/artifact/smoke/snapshot/audit/seo/a11y/links/report helpers、browser task packs、workflow packs、workflow YAML init/doctor/plan/run、run triage、run inspect、run bundle evidence export、config explanation、config doctor、可搜索 HTML trace export、`traces open`、`report latest`、带可复制命令的增强版 `dashboard`、dashboard workflow/template recommendation sections、`report trend`、latest-report HTML export、experimental OTel trace export、connector diagnostics、本地 skill catalog install、plugin doctor、offline mock runs、starter templates、coding/research/data/ops/browser/web-monitor/SEO/website-audit/GitHub-issue/daily-report/competitor-research/meeting-notes/content-pipeline/support/sales/document/QA/release starters 的 `new <scenario>`、Python `AgentApp` workflow/browser/bundle helpers、quickstart、run explanation、advice-only JSON/Markdown/HTML run fix packages、browser-MCP failure classification、run listing、run summary、structured trace tree export、executor capability reports、storage contracts 与 real-network scenario proof metadata。
