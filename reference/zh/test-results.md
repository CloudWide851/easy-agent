...
- 静态检查：`ruff` 与 `mypy`
- 定向回归：setup preflight、wizard、config explanation、config doctor、可搜索 HTML trace export、本地 trace 打开、latest-report 汇总和 HTML export、静态 dashboard export、suggested dashboard next steps、可复制 dashboard inspect/bundle 命令、dashboard workflow/template recommendations、report trend、cost reports、只读 console dry run、experimental OTel trace export、connector diagnostics、MCP 静态 doctor/test、federation graph export、workflow packs、workflow YAML init/doctor/validate/explain/plan/run、run triage/inspect/notes/bundle、Playwright MCP browser readiness、browser doctor/artifact/smoke/snapshot/audit/seo/a11y/links/report helpers、browser task packs、本地 skill catalog、plugin doctor、Python facade workflow/browser/bundle/dashboard/cost embedding、mock provider、onboarding CLI、scenario creation、新业务模板、run explanation/fix packages、provider compatibility、config validation、guardrails、BFCL evaluation、official-source search 与 simple-evals profile support；最新 focused set 为 `33 passed`，保留兼容性集合为 `55 passed`、`23 passed`、`36 passed`、`89 passed` 加 `4 passed`
- 全量 unit tests：`233 passed`
...
- 新增 focused regressions 覆盖 setup preflight、wizard scenario creation、模板 `workflow.yml` 生成、template metadata filters/show/recommendation、browser template Playwright MCP config、browser doctor/artifact/smoke/snapshot/audit/seo/a11y/links/report helpers、browser task packs、workflow packs、workflow YAML init/doctor/validate/explain/plan/run、MCP 静态 doctor/test、federation graph export、run triage、run inspect Markdown/HTML export、run notes、run bundle evidence export、config explanation、config doctor、可搜索 HTML trace export、`traces open`、`report latest`、`report costs`、带 cost 与可复制命令的增强版 `dashboard`、只读 `console --dry-run`、dashboard workflow/template recommendation sections、`report trend`、latest-report HTML export、experimental OTel trace export、connector diagnostics、本地 skill catalog install、plugin doctor、offline mock runs、starter templates、coding/research/data/ops/browser/web-monitor/SEO/website-audit/website-release/API-regression/incident-review/GitHub-issue/GitHub-PR/daily-report/weekly-report/data-quality/competitor-research/meeting-notes/content-pipeline/support/sales/document/QA/release starters 的 `new <scenario>`、Python `AgentApp` workflow/browser/bundle/dashboard/cost helpers、quickstart、run explanation、advice-only JSON/Markdown/HTML run fix packages、browser-MCP failure classification、run listing、run summary、structured trace tree export、executor capability reports、storage contracts 与 real-network scenario proof metadata。
- CLI smoke 已覆盖 `workflow list`、`workflow init`、`workflow doctor`、`workflow plan`、`workflow run workflow.yml --dry-run`、`workflow run --dry-run`、`runs triage`、`runs inspect`、`runs bundle`、增强版 `dashboard`、`browser smoke`、`browser snapshot`、`browser audit`、`browser seo`、`browser a11y`、`browser links`、`browser report`、`browser doctor`、`browser artifacts`、`connectors doctor`、`connectors test browser`、`task show repo-review`、`task show browser-qa`、`report trend`、`skills catalog list` 与 `new web-monitor-agent`。
