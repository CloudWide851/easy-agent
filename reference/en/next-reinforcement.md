...
- Treat the new wizard, connector diagnostics, template metadata/recommendation market, workflow YAML doctor/validate/explain/plan/run, `runs inspect`, run notes, `runs bundle`, browser smoke/snapshot/audit/seo/a11y/links/report helpers, browser doctor/artifact inspection, MCP static doctor/test, federation graph export, browser-specific task packs, advice-only HTML run fix packages, cost reports, read-only local console, static dashboard workflow/template recommendations with copyable commands, local skill catalog workflows, Python `AgentApp` workflow/browser/bundle/dashboard/cost helpers, and federation-demo checks as the next operator-facing usability layer before adding heavier runtime dependencies.
...
- make `runs inspect` the default next step after failed runs, including Markdown/HTML export, diagnostic codes, notes, repair prompts, and cost summary; keep `runs explain` available for raw classifier output, use `runs triage` for compact severity/actionability, use `runs fix` when the user needs a packaged advice-only repair prompt, and use `runs bundle` when the user needs a complete handoff directory containing run summary, triage, fix, trace, and browser artifact evidence
...
- keep browser work MCP-first: `browser-agent`, `web-monitor-agent`, `seo-agent`, `website-audit-agent`, and `competitor-research-agent` now use `browser.enabled: true` with Playwright MCP, default isolated/headless execution, local browser artifacts, browser doctor/artifact/smoke/snapshot/audit/seo/a11y/links/report commands, and approval-gated sensitive browser actions; next work should harden catalog drift, artifact lifecycle, page-quality audit evidence, and browser-specific approval UX rather than adding a second native browser stack prematurely
...
- use `template list --tag/--risk`, `template show`, and `template recommend` as the local starter-market entrypoint, so API regression, website release, incident review, PR review, reporting, and data-quality starters are discoverable by intent instead of by memorized names
- use `workflow list`, `workflow show`, `workflow init`, `workflow doctor`, `workflow validate`, `workflow explain`, `workflow plan`, and `workflow run` as the operator-facing wrapper around task packs, with portable `workflow.yml` files, static preflight checks, strict validation, explainable risk summaries, optional bundle-on-completion, and next commands visible before model-backed execution
- use `runs inspect` as the short failure-to-next-step path in dashboard suggestions, browser reports, and workflow results
- use `dashboard` as a static local operations page before adding a persistent web server, because it keeps the operator view dependency-free and easy to archive with run evidence while surfacing failed runs, approvals, browser readiness, browser artifacts, workflow recommendations, template recommendations, cost/reliability summaries, and copyable inspect/bundle commands together
- use `report costs` and `console --dry-run` as the lightweight cost/reliability and local-operations shell before introducing any persistent service or database-backed console
- keep `task run --dry-run`, `workflow plan`, and `workflow run --dry-run` useful for prompt review and approvals before a task is sent to a model-backed agent
...
- use `mcp doctor`, static `mcp test`, and `federation graph` as low-risk topology checks before live MCP startup or remote federation traffic
