...
- Treat the new wizard, connector diagnostics, workflow YAML doctor/plan/run, `runs inspect`, `runs bundle`, browser smoke/snapshot/audit/seo/a11y/links/report helpers, browser doctor/artifact inspection, browser-specific task packs, advice-only HTML run fix packages, static dashboard workflow/template recommendations with copyable commands, local skill catalog workflows, Python `AgentApp` workflow/browser/bundle helpers, and federation-demo checks as the next operator-facing usability layer before adding heavier runtime dependencies.
...
- make `runs inspect` the default next step after failed runs, keep `runs explain` available for raw classifier output, use `runs triage` for compact severity/actionability, use `runs fix` when the user needs a packaged advice-only repair prompt, and use `runs bundle` when the user needs a complete handoff directory containing run summary, triage, fix, trace, and browser artifact evidence
...
- use `workflow list`, `workflow show`, `workflow init`, `workflow doctor`, `workflow plan`, and `workflow run` as the operator-facing wrapper around task packs, with portable `workflow.yml` files, static preflight checks, optional bundle-on-completion, and next commands visible before model-backed execution
- use `runs inspect` as the short failure-to-next-step path in dashboard suggestions, browser reports, and workflow results
- use `dashboard` as a static local operations page before adding a persistent web server, because it keeps the operator view dependency-free and easy to archive with run evidence while surfacing failed runs, approvals, browser readiness, browser artifacts, workflow recommendations, template recommendations, and copyable inspect/bundle commands together
