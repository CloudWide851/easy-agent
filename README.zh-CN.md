...
- 内置 BFCL 子集与 tau2 mock 子集的 public evaluation 能力，并增加了带阶段感知的 fallback 分析、failure buckets，以及面向 OpenAI-compatible schema 失败路径的 provider 级兼容矩阵。
...
这份快照合并了 2026-04-09 重新完成的完整 Python 验证。real-network、public-eval 与 benchmark 三类工件都在本轮重新刷新。
...
| 完整 unit suite | `.\.venv\Scripts\python.exe -m pytest tests/unit -q --basetemp=%TEMP%\easy-agent-pytest\unit-full-<timestamp>` | `117 passed` |
| 完整 real integration suite | `.\.venv\Scripts\python.exe -m pytest tests/integration -m real -q --basetemp=%TEMP%\easy-agent-pytest\integration-full-<timestamp>` | `5 passed`，附带 4 条已知 Windows 清理告警 |
...
| Live real-network 工件 | `.easy-agent/real-network-report.json` | 已于 2026-04-09 刷新 |
| Live public-eval 工件 | `.easy-agent/public-eval-report.json` | 已于 2026-04-09 刷新 |
| Live benchmark 工件 | `.easy-agent/benchmark-report.json` | 已于 2026-04-09 刷新 |

live public-eval 工件通过仓库内 Python helper 调用 `run_public_eval_suite('easy-agent.yml')` 刷新；benchmark 工件通过 `scripts/benchmark_modes.py` 刷新；real-network 工件则通过本轮 live 验证里的 focused Python real-network suite 刷新。

### 真实网络矩阵

| 场景 | 传输 | 状态 | 耗时 (s) | 说明 |
| --- | --- | --- | --- | --- |
| `cross_process_federation` | `http_poll` | passed | `1.4032` | 跨进程 well-known discovery 与 send/poll 联邦链路通过 |
| `live_model_federation_roundtrip` | `http_poll` | passed | `7.4190` | live-model loopback federation 在 API key、TCP 连通性和 HTTP 可达性预检之后成功跑通 |
| `disconnect_retry_chaos` | `http_webhook` | passed | `4.1993` | callback 重试、`pushNotificationConfig`、`sendSubscribe`、签名 webhook 交付与 resubscribe 链路通过 |
| `duplicate_delivery_replay_resilience` | `http_webhook` | passed | `3.7237` | 重复投递、签名 callback replay 与稳定 federated task event log 读取链路通过 |
| `workbench_reuse_process` | `local_process` | passed | `1.9306` | process workbench 成功复用同一个长生命周期 session root |
| `workbench_reuse_container` | `podman_exec` | passed | `29.5252` | container executor 成功加载 offline image archive、施加配额并从 snapshot image 恢复 |
| `workbench_incremental_snapshot_reuse_container` | `podman_exec` | passed | `48.4886` | container 多次 checkpoint restore 后仍能保留增量状态 |
| `workbench_reuse_microvm` | `podman_machine_ssh` | passed | `17.2082` | microVM executor 通过 podman machine over SSH 复用并在断线式重启后恢复 |
| `workbench_incremental_snapshot_reuse_microvm` | `podman_machine_ssh` | passed | `25.3954` | microVM 多次 checkpoint restore 后仍能保留增量状态 |
| `replay_resume_failure_injection` | `sqlite_checkpoint` | passed | `5.5155` | 注入失败后的 resume、replay、fork 恢复链路通过 |

汇总：`10 passed`、`0 failed`、`0 skipped`。
来源：`.easy-agent/real-network-report.json`，生成时间 `2026-04-09T07:10:12Z`。
...
来源：`.easy-agent/benchmark-report.json`，已于 2026-04-09 刷新。
...
| `overall.bfcl_pass_rate` | `0.9167` | 当前剩余失败已经收敛到 1 个 duplicate-call case 和 1 个 optional selector case |

### 带阶段感知的 Public Eval 分析

| 指标 | 数值 | 说明 |
| --- | --- | --- |
| Base 阶段终态通过率 | `0.9259` | 27 个用例里有 25 个在不离开 base stage 的情况下完成 |
| Retry 阶段迁移数 | `0` | 本次 live pass 没有触发 `strict_schema_retry` 或 `candidate_pruned_retry` |
| Failure buckets | `duplicate_call=1`、`other=1` | 刷新后的 live 报告里 `schema_or_provider_failure=0`、`history_grounding_miss=0` |

### Provider Schema 兼容矩阵

| Provider | Protocol | 兼容结论 | 说明 |
| --- | --- | --- | --- |
| `openai_compatible` | `openai` | 已规范化、strict 化并裁剪 | 会扁平化 root object alias、tuple array 与 `anyOf`，发出 `strict: true`，递归设置 `additionalProperties: false`，把 optional 字段提升为 required nullable 字段，并暴露 `parallel_tool_calls` 控制 |
...
来源：`.easy-agent/public-eval-report.json`，已于 2026-04-09 刷新。

当前注意事项：

- 完整 real integration suite 在成功后仍会出现 Windows asyncio subprocess cleanup warnings；在这台机器上它们依然是清理债务，而不是功能失败。
- 刷新后的 live public-eval 里，剩余 BFCL miss 已经收敛到 1 个 duplicate multi-intent case，以及 1 个 optional selector case（`root_type='all'`），而不是 OpenAI-compatible provider schema 失败。
- 刷新的 benchmark 已经全绿，但 `sub_agent` 路径仍显著慢于其他 benchmark 模式，因为它同时支付了 live coordinator turn 和 live delegated turn 的成本。

## 下一步补强

这些下一步补强方向是根据当前公开的 A2A、MCP 和 OpenAI tool-calling 能力面整理出来的，而不是只来自仓库内部 backlog。

- 继续把 federation 贴近最新公开 A2A 面，补齐更丰富的 agent-card 元数据、signed-card 校验、push-notification 鉴权细节、task history 过滤能力，以及 status timestamp 一致性，让 remote inspect 与 replay 更贴近协议契约。
- 把联邦信任模型继续推进到 readiness check 之后，包括真正的一等公民 OAuth/OIDC token 获取与刷新、针对 signed card / callback 的 JWKS/JWS 校验，以及更严格的按 tenant / task 划分的授权边界。
- 围绕 OpenAI 官方 function calling 与 structured outputs 约束继续补强 provider 兼容性，增加 `strict: true`、`additionalProperties: false`、nullable 与 optional 参数建模，以及单调用 / 并行调用约束的回归覆盖。
...
- 把 OpenAI-compatible 的调用控制面继续从 `tool_choice='auto'` 扩展到 forced-none、forced-single-tool 与串行多步 tool plan 的回归覆盖，并继续复用同一套 strict schema 路径。
- 把 MCP roots 从当前的 list 获取继续推进到完整的 `notifications/roots/list_changed` 传播与 root-diff 协调，并把 URL mode elicitation completion 以及显式 `accept` / `decline` / `cancel` 结果纳入同一套 durable approval state。
- 把当前 public-eval 的 stage summary、failure buckets 和 provider schema matrix 继续推进成跨轮次可比对的趋势工件，而不只是停留在最新一次快照里。
- 为 container / microVM warm-start 路径增加延迟预算和 cache telemetry，让 real-network 矩阵除了 pass/fail 之外，还能长期跟踪恢复速度与 snapshot drift。
