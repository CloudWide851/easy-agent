...
- `agent-card` 与 `extended-agent-card` 现在会暴露 protocol version、card schema version、modalities、declared capabilities、auth hints、retry policy、subscribe policy、well-known endpoints 和 compatibility metadata。
...
快照日期：2026 年 3 月 29 日。

这份快照合并了 2026 年 3 月 29 日重新完成的 Python 侧验证结果，以及 `.easy-agent/` 下当前保留的 benchmark / public-eval 工件。新一轮验证使用了 Python `3.12.11`、本地 `.env.local` 凭据、真实 DeepSeek 调用、仓库当前的真实 MCP 集成测试，以及用于 executor 覆盖的本地 Podman machine。
...
| 单元测试 | `.\.venv\Scripts\python.exe -m pytest tests/unit -q --basetemp=%TEMP%\easy-agent-pytest\unit-full-<timestamp>` | `76 passed` |
| 专项 real-network pytest | `.\.venv\Scripts\python.exe -m pytest tests/integration/test_real_network_eval.py -q -m real --basetemp=%TEMP%\easy-agent-pytest\integration-real-network-<timestamp>` | `1 passed` |
| 真实集成测试 | `.\.venv\Scripts\python.exe -m pytest tests/integration -m real -q --basetemp=%TEMP%\easy-agent-pytest\integration-full-<timestamp>` | `4 passed`、`1 skipped` |
| Live benchmark 工件 | `.easy-agent/benchmark-report.json` | 复用现有快照 |
| Live public-eval 工件 | `.easy-agent/public-eval-report.json` | 复用现有快照 |
| Live real-network 刷新 | 调用 `run_real_network_suite()` 的 Python 辅助脚本 | 已刷新并得到 `6 passed` |
...
| 场景 | 传输 | 状态 | 耗时 (s) | 说明 |
| --- | --- | --- | --- | --- |
| `cross_process_federation` | `http_poll` | passed | `1.4692` | 跨进程 well-known discovery 与 send/poll 联邦链路通过 |
| `disconnect_retry_chaos` | `http_webhook` | passed | `4.1902` | callback 重试、`pushNotificationConfig`、`sendSubscribe` 与 resubscribe 链路通过 |
| `workbench_reuse_process` | `local_process` | passed | `1.5789` | process workbench 成功复用同一个长生命周期 session root |
| `workbench_reuse_container` | `podman_exec` | passed | `29.6954` | container executor 成功加载离线镜像归档、施加资源配额，并从 snapshot image 恢复 |
| `workbench_reuse_microvm` | `podman_machine_ssh` | passed | `19.1149` | microvm executor 通过 podman machine SSH 成功恢复断线后的 workbench 会话 |
| `replay_resume_failure_injection` | `sqlite_checkpoint` | passed | `5.6087` | 注入失败后的 resume、replay、fork 恢复链路通过 |

汇总：`6 passed`、`0 failed`、`0 skipped`。
来源：`.easy-agent/real-network-report.json`，生成时间 `2026-03-29T15:05:37Z`。
...
来源：当前保留的 `.easy-agent/public-eval-report.json`，最近一次校验来自 2026-03-27 的 live verification 轮次。

当前注意事项：

- 完整的 `tests/integration -m real` 在这台机器上仍有 1 个宿主机侧 skip，因为本地 Redis 没有监听 `127.0.0.1:6379`，所以 long-run real suite 的 Redis 路径没有在本轮执行。
- live suite 结束后仍会出现 Windows `asyncio` 子进程清理 warning，但真实测试和报告生成都已完成。
- DeepSeek 的 OpenAI-compatible endpoint 在 BFCL multiple 与 irrelevance 子集上仍会返回一部分 provider-side `400 Bad Request`，即便现在已经增加了更强的 schema flattening 和更严格的工具调用提示。

## 下一步补强

这些下一步补强方向是根据当前公开的 A2A 与 MCP 协议面整理出来的，而不是只来自仓库内部 backlog。

- 继续把 federation card 协商贴近最新公开 A2A 面，补齐更丰富的 artifact / part 级模态声明、`ListTasks` 的 cursor 分页（`pageToken` / `nextPageToken`），以及更清晰的 notification compatibility metadata。
- 把联邦安全继续推进到生产级远端信任模型，包括更强的 auth scheme hints、callback 签名校验、OAuth / OIDC 流程，以及可选的 agent server 之间 mTLS。
- 把 MCP 能力协商从“支持 roots”继续推进到完整的 `roots/list_changed` 流程，并加强 `streamable_http` 下的断线重连、鉴权刷新和 server-initiated lifecycle 处理。
- 把 MCP sampling 与 elicitation 的策略做得更细，区分低风险与高风险远程请求，并支持更完整的 form / URL elicitation 结果处理，同时不绕过 human approval。
- 增加 provider-aware 的 public-eval fallback 策略，让复杂 BFCL schema 在 OpenAI-compatible `400` 场景下可以更平滑降级，而不是直接失败。
- 继续扩展真实网络评测矩阵，加入 live-model 联邦、重复投递或 replay 韧性校验，以及更快的 container / microVM 增量快照复验链路。
