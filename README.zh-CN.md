...
- web-search 评测补强：SerpApi `/search.json`、replay-backed contents、quota ledger、result grounding、raw official BFCL manifest 归一化、single-call regression guards
...
当前已发布的小版本仍然是 `0.3.4`。本轮未发布补强继续沿用 2026 年 4 月 9 日的 benchmark 快照与 2026 年 4 月 13 日的公开分数基线，同时把运行时推进到 OpenAI Responses API 对齐、raw `official_full_v4` manifest 归一化，以及更深入的 MCP template refresh 协调。Python 验证套件已在 2026 年 4 月 14 日重新执行，但没有改动公开分数基线。方法说明、公开对比与详细矩阵见 [reference/zh/test-results.md](./reference/zh/test-results.md)。
...
- 继续把 raw official BFCL v4 覆盖扩展到更广的 agentic 与 multihop 回归，并补齐更清晰的官方分类诊断
