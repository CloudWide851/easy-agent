...
- 新增离线 `mock` provider 以及 `setup`、`init`、`quickstart`、`template create`、`config explain`、`config doctor`、`runs explain` 和可搜索 HTML trace export，可以零凭据完成上手、配置理解和失败归因。
...
当前已发布的小版本仍然是 `0.3.5`。README 里保留的 benchmark 与 public-eval headline 分数仍以 2026 年 4 月 14 日发布快照为基线，而 2026 年 4 月 29 日这轮最新的 Python 验证重新跑通了 `ruff`、`mypy`、`214` 个 unit tests 和 `7` 个 live integration tests，但没有改写这组保留分数基线。方法说明、公开对比与详细矩阵见 [reference/zh/test-results.md](./reference/zh/test-results.md)。
...
- 把已经交付的 structured trace tree 作为主要排障入口，并继续稳定字段，为后续 OpenTelemetry 风格导出做准备
- 继续用 guided setup preflight、config explanation、config risk checks、scenario templates 与可搜索 HTML trace inspection 固化零凭据上手层
