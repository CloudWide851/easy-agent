...
- 新增离线 `mock` provider 以及 `setup`、`wizard`、`init`、`quickstart`、场景模板、connector diagnostics、task packs、`config doctor`、`runs explain`、advice-only `runs fix`、`traces open`、`report latest`、`report trend`、单文件 HTML trace/report/fix/dashboard export 和轻量 Python `AgentApp` facade，可以零凭据完成上手、配置理解和失败归因。
...
当前已发布的小版本仍然是 `0.3.5`。README 里保留的 benchmark 与 public-eval headline 分数仍以 2026 年 4 月 14 日发布快照为基线，而 2026 年 4 月 30 日这轮最新的 Python 验证重新跑通了 `ruff`、`mypy`、`230` 个 unit tests 和 `7` 个 live integration tests，但没有改写这组保留分数基线。方法说明、公开对比与详细矩阵见 [reference/zh/test-results.md](./reference/zh/test-results.md)。
...
- 继续用 guided setup/wizard preflight、config explanation、connector diagnostics、browser doctor/artifact inspection、task packs、静态 dashboard、advice-only HTML run repair package，以及面向 coding、research、data、ops、browser automation、support、sales、documents、QA、release checks 的业务模板固化零凭据上手层
