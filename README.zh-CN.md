...
- 新增离线 `mock` provider 以及 `setup`、`init`、`quickstart`、`template create`、`config explain`、`runs explain` 和 HTML trace export，可以零凭据完成上手、配置理解和失败归因。
...
- 把原始 event stream 继续整理成结构化 trace tree，并补齐 run list、run summary 与 trace export 入口
- 把新的零凭据上手层继续扩展成更完整的 guided setup、config explanation、scenario templates 与 HTML trace inspection
