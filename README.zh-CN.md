...
- web-search 评测补强：SerpApi `/search.json`、grounded source ledger、cache-first contents reuse、replay-backed contents fallback、raw official BFCL manifest 归一化、single-call regression guards
...
当前已发布的小版本是 `0.3.5`。这次 release 在 2026 年 4 月 14 日同步刷新了 benchmark、public-eval 与 real-network 快照，并把运行时继续推进到 grounded web-search source ledger、cache-first grounded contents recovery，以及更严格的 Responses / structured-output 回归覆盖。方法说明、公开对比与详细矩阵见 [reference/zh/test-results.md](./reference/zh/test-results.md)。
...
- 把已交付的 chat-completions / Responses API 双路径对齐继续推进成 live provider-specific 兼容证据
- 继续把 BFCL web-search 从 grounded replay safety 推进到更丰富的 source-ledger、source-aware 与 multihop 官方覆盖
